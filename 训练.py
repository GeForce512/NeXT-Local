...
class Monitor(TrainerCallback):
    def __init__(s, log):
        s._log, s.steps, s.losses = log or print, [], []
    ...
    def on_log(s, a, state, c, logs=None, **k):
        if logs and 'loss' in logs:
            ...
            # ★ 核心：实时推送到前端
            s._log(
                f"📊 Step {state.global_step} | Loss: {loss:.4f} | LR: {logs.get('learning_rate', 0):.2e} | Epoch: {logs.get('epoch', 0):.3f}")
...
def run_training(log_fn=None, config=None):
    log = log_fn or print
    ...
    def fmt(ex):
        ...
        for m in ex['messages']:
            ...
            ids.append(f_ids[:MAX_SEQ_LENGTH]);
            masks.append([1] * len(f_ids[:MAX_SEQ_LENGTH]));
            lbs.append(f_lbs[:MAX_SEQ_LENGTH])
    ...
    args = TrainingArguments(output_dir=out_dir, per_device_train_batch_size=BATCH_SIZE,
                             gradient_accumulation_steps=GRAD_ACC, num_train_epochs=EPOCHS, learning_rate=LEARNING_RATE,
                             warmup_ratio=0.1, lr_scheduler_type="cosine", bf16=True, logging_steps=1,
                             save_strategy="no", optim="paged_adamw_8bit", disable_tqdm=True)
    trainer = Trainer(model=model, args=args, train_dataset=ds_obj,
                      data_collator=DataCollatorForSeq2Seq(tok, padding=True), callbacks=[Monitor(log)])
