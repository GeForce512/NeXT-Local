...
- 保留原生进度条与深度验证报告
"""
import os, sys, subprocess, urllib.request, urllib.parse, zipfile, tempfile, time, shutil, ssl, json, importlib
import os
...
REQUIRED_PACKAGES = {
    "transformers": "5.13.1", "accelerate": "1.14.0", "datasets": "5.0.0",
    "tokenizers": "0.22.2", "peft": "0.19.1", "bitsandbytes": "0.49.2",
    "safetensors": "0.8.0", "PyQt5": "5.15.11", "PyQtWebEngine": "5.15.7",
    "flask": "3.1.3", "flask-cors": "6.0.5", "numpy": "2.2.6",
    "pillow": "12.2.0", "tqdm": None, "requests": None,
    "modelscope": "1.22.0", "huggingface_hub": "1.23.0"
}
...
# ================= ★ PyTorch CUDA 防覆盖验证 =================
def verify_torch_cuda(python_exe, log=None):
    """确保 torch 仍为 CUDA 版本（pip 依赖可能覆盖为 CPU 版）"""
    log = log or print
    log("\n🔒 验证 PyTorch CUDA 支持...")
    # ★ 写临时脚本避免 Windows cmd.exe 引号问题
    check_script = os.path.join(tempfile.gettempdir(), '_next_torch_check.py')
    with open(check_script, 'w', encoding='utf-8') as f:
        f.write("import json,sys\nr={'ok':False,'ver':'?','cuda':None}\ntry:\n import torch\n r['ver']=torch.__version__;r['cuda']=torch.version.cuda;r['ok']=torch.cuda.is_available()\nexcept:pass\nprint(json.dumps(r))\n")
    ok, out, _ = run_cmd(f'"{python_exe}" "{check_script}"', timeout=30)
    if not ok:
        log("⚠️ 无法检测 torch 状态")
        return False
    try:
    ...
    ver = info.get("ver", "?")
    cuda = info.get("cuda")
    log(f"   torch={ver}, CUDA={cuda}, GPU={'OK' if info.get('ok') else 'NO'}")

    if info.get("ok"):
        log("✅ PyTorch CUDA 完好")
        return True

    log("⚠️ PyTorch CUDA 丢失！正在强制重装 cu121 版本...")
    pkg_str = f"torch=={PYTORCH_VERSION} torchvision=={TORCHVISION_VERSION} torchaudio=={TORCHAUDIO_VERSION}"
    anti_stuck = "--progress-bar=on --timeout 1000 --retries 10 --force-reinstall --no-deps"

    for pt_url in PYTORCH_INDEX_URLS:
        mirror_name = "阿里云" if "aliyun" in pt_url else ("南京大学" if "nju" in pt_url else "官方")
        trusted_host = urllib.parse.urlparse(pt_url).netloc
        cmd = (f'"{python_exe}" -m pip install {pkg_str} '
               f'--index-url {pt_url} --trusted-host {trusted_host} {anti_stuck}')
        if run_cmd(cmd, capture=False, timeout=1200)[0]:
            log(f"✅ [{mirror_name}] PyTorch cu121 重装成功")
            return True
        log(f"⚠️ [{mirror_name}] 重装失败")

    log("❌ PyTorch CUDA 重装失败")
    return False


# ================= 深度验证 =================
def deep_verify_env(python_exe, log=None):
    verify_script_path = os.path.join(tempfile.gettempdir(), '_next_verify_env.py')
    with open(verify_script_path, 'w', encoding='utf-8') as f:
        f.write("import sys, importlib, os\n")
        f.write("os.environ['QT_QPA_PLATFORM'] = 'offscreen'\n")
        f.write("passed = total = 0\n")
        f.write("def check(name, extra=None):\n")
        f.write("    global passed, total; total += 1\n")
        f.write("    try:\n")
        f.write("        mod = importlib.import_module(name)\n")
        f.write("        ver = getattr(mod, '__version__', 'core')\n")
        f.write("        if extra: extra(mod)\n")
        f.write("        print(f'OK {name:<25} [ver: {ver}]'); passed += 1\n")
        f.write("    except Exception as e: print(f'FAIL {name:<25} [{str(e)[:40]}]')\n")
        f.write("print('=== 1. GPU ===')\n")
        f.write("def chk(torch):\n")
        f.write("    if torch.cuda.is_available(): print(f'  GPU: {torch.cuda.get_device_name(0)} ({torch.cuda.get_device_properties(0).total_memory / 1024**3:.1f}GB)')\n")
        f.write("    else: print('  CPU only (WARNING)')\n")
        f.write("check('torch', chk)\n")
        f.write("print()\n")
        f.write("print('=== 2. Core libs ===')\n")
        f.write("for lib in ['transformers', 'peft', 'bitsandbytes', 'PyQt5', 'flask']: check(lib)\n")
        f.write("print()\n")
        f.write("print('='*60)\n")
        f.write("print(f'Done: {passed}/{total} passed.')\n")
        f.write("print('VERIFY_SCRIPT_DONE')\n")
    success, stdout, stderr = run_cmd(f'"{python_exe}" "{verify_script_path}"', timeout=60)
    log("\n" + "=" * 60 + "\n🚀 验证报告\n" + "=" * 60)
    if success and "VERIFY_SCRIPT_DONE" in stdout:
        for line in stdout.split('\n'):
            if any(k in line for k in ['OK ', 'FAIL ', 'GPU:', '===', 'Done:', 'CPU']): log(line)
        return True
    return False
...
def run_env_setup(log_fn=None):
    ...
    # ★ PyTorch CUDA 防覆盖：普通包安装后验证 torch 仍为 cu121 版本
    if not verify_torch_cuda(PYTHON_EXE, log=log):
        log("❌ PyTorch CUDA 验证失败，环境不可用")
        return False
